...
public class CodeDisplay extends ElementComposite
  ...
  public CodeDisplay setText(String text) {
    ...
    String scr =
        "if (typeof Prism != 'undefined' && typeof whenPrismLang"
            + language
            + "Loaded != 'undefined') {whenPrismLang"
            + language
  ...
  public CodeDisplay setLanguage(String language) {
    if (isSupportedLanguage(language)) {
      ...
      scr =
          "function whenPrismLang"
              + language
              + "Loaded (callback) { if (typeof Prism.languages."
              + language
              + " === 'undefined' || typeof Prism === 'undefined') {setTimeout (function () {whenPrismLang"
              + language
              + "Loaded (callback);}, 100);} else { callback (); }}";
      Page.getCurrent().addInlineJavaScript(scr);
      ...
      scr =
          "whenPrismLoaded(function() {var link2 =  document.createElement('script');link2.setAttribute('type','module');link2.setAttribute('src','"
              + url
              + "');"
              + "document.head.appendChild(link2);whenPrismLang"
              + language
              + "Loaded(function() {Prism.highlightAll();}) });";
      self.executeJs(scr);
