...
public class DateFieldView extends Composite<FlexLayout> {
  ...
  private void syncDates(ValueChangeEvent<LocalDate> e) {
    ...
    if (ret.isBefore(dep)) {
      if (e.getSource() == departureField) returnField.setValue(dep);
      else departureField.setValue(ret);
    }
  }

  private LocalDate clamp(LocalDate date) {
    if (date == null) return null;
    return date.isBefore(TODAY) ? TODAY : date.isAfter(MAX_DATE) ? MAX_DATE : date;
  }
}
