...
public class RefresherThemesView extends Composite<Div> {
  ...
  public RefresherThemesView() {
    Div canvas =
        new Div().addClassName("is-canvas").setAttribute("theme", Theme.INFO.name().toLowerCase());

    Refresher refresher = new Refresher().setTheme(Theme.INFO);
  ...
  class Item extends Composite<Div> {
    public Item() {
      ...
      Icon icon = FeatherIcon.ARROW_RIGHT.create().setMinSize("24px", "24px");

      itemSelf.add(
          FlexLayout.create(FlexLayout.create(nameDiv, excerpt).vertical().build(), icon)
              .horizontal()
              .justify()
              .between()
              .align()
