...
@Route
public class AppNavPinningView extends Composite<AppLayout> {
  private final AppLayout self = getBoundComponent();
  private final Toolbar toolbar = new Toolbar();
  private final AppNav appNav = new AppNav();

  public AppNavPinningView() {
    setHeader();
    setPinning();
    setDrawer();
  }
  ...
  private AppNavItem createItem(String text, String icon) {
    return new AppNavItem(
        text, AppNavPinningPageView.class, ParametersBag.of("id=" + text), TablerIcon.create(icon));
  }
}
