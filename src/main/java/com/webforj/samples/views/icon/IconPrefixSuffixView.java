...
public class IconPrefixSuffixView extends Composite<FlexLayout> {
  ...
  public IconPrefixSuffixView() {
    ...
    Button nextButton = new Button("Next").setSuffixComponent(arrow).setTheme(ButtonTheme.PRIMARY);

    Icon filter = TablerIcon.create("filter");
    Button filterButton =
        new Button("Filter").setPrefixComponent(filter).setTheme(ButtonTheme.DEFAULT);
