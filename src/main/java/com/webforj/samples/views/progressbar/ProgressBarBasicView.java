...
public class ProgressBarBasicView extends Composite<Div> {
  ...
  private final ProgressBar bar = new ProgressBar("Progress {{x}}% - value: {{value}}");
  private final FlexLayout buttonContainer = FlexLayout.create(start, pause, reset).build();
  ...
  public ProgressBarBasicView() {
    ...
    bar.setStriped(true).setAnimated(true);
