...
public class CompositePage {
  ...
  public CompositePage(Page page) {
    this.toDoInput =
        page.getByRole(
            AriaRole.TEXTBOX,
            new Page.GetByRoleOptions().setName("Enter a new task and press Enter..."));
    this.firstTaskCheckbox =
        page.getByRole(
            AriaRole.CHECKBOX, new Page.GetByRoleOptions().setName("Review documentation"));
    this.firstDeleteButton =
        page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("Delete")).first();
  }
