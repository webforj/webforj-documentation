...
public class DrawerTaskViewIT extends BaseTest {
  ...
  public void testTaskAddedWhenAddTaskButtonIsClicked() {
    ...
    Locator meetingCheckbox =
        page.getByRole(
            AriaRole.CHECKBOX, new Page.GetByRoleOptions().setName("Call John about the meeting"));
    meetingCheckbox.check();

    Locator clearButton =
        page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("Clear Completed"));
    clearButton.click();

    assertThat(meetingCheckbox).not().isVisible();

    Locator newTaskInput =
        page.getByRole(AriaRole.TEXTBOX, new Page.GetByRoleOptions().setName("New Task"));
