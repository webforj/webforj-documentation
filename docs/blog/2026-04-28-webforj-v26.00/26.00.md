...
## Design System 2.0 {#design-system-20}
...
Install the [webforJ AI plugin](/docs/ai-tooling/agent-skills) and ask your assistant *"Theme this app with a blue palette,"* or *"Style this dwc-button to match our brand guidelines,"* and the `webforj-styling-apps` skill takes it from there.

</Admonition>

## webforJ AI plugin {#webforj-ai-plugin}

New alongside `26.00`: the **webforJ AI plugin**, a one-install bundle that teaches your AI coding assistant tons of webforJ details and specifics. It ships the [webforJ MCP server](/docs/ai-tooling/mcp), which gives live access to docs, project scaffolding, theme generation, and design-token validation, together with the platform's [Agent Skills](/docs/ai-tooling/agent-skills) that tell the assistant *when* to reach for each of those tools and *how* to use them effectively.
...
:::info Learn More
See the [webforJ AI plugin](/docs/ai-tooling) documentation for per-client install steps.
:::

## Dialog auto width {#dialog-auto-width}
...
## TabbedPane segment control {#tabbedpane-segment-control}
...
### Java 21 minimum {#java-21-minimum}
...
## Automated migration with `webforj-rewrite` {#automated-migration-with-webforj-rewrite}

<Admonition type="tip" icon={<AutoAwesomeIcon fontSize="small" />} title="Building with AI?">

Install the [webforJ AI plugin](/docs/ai-tooling/agent-skills) and ask your assistant *"Upgrade this app from webforJ 25 to 26,"* or *"Run the rewrite recipe and resolve the TODOs,"* and the `webforj-upgrading-versions` skill takes it from there.
