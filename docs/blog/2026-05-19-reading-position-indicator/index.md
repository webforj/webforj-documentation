---
title: "A webforJ Reading Position Indicator"
...
Recently, I was browsing articles on [CSS Tricks](https://css-tricks.com) and came across an article by Pankaj Parashar titled "[Reading Position Indicator](https://css-tricks.com/reading-position-indicator/)."
I've seen this type of indicator in articles, blogs, and in lengthy terms and conditions that I've definitely read through thoroughly. What I wanted to try was recreating a reading position indicator using webforJ, and see if I could build it in less than 100 lines of code. 
...
![gif showcasing the webforJ reading position indicator](/img/tempReadingPositionIndicator.gif)

<!-- vale off -->
## Adding the visual aspects
<!-- vale on -->
### The main component

I knew immediately which webforJ component I wanted to use for this project, the [`ProgressBar`](/docs/components/progressbar) component. Even though it's the most crucial component that made my idea work, it only took a few lines to get it working the way I wanted.
...
To keep it anchored, I decided to style my toolbar with CSS, targeting the web component as it appears in the DOM. Using two `setStyle()` calls for anchoring the toolbar could have saved me some lines of code, as I wouldn't have to use the `@StyleSheet` annotation or wrap my CSS styling in a selector, but I  ultimately decided to keep this project organized. I'm glad I made that design choice because I decided later into the project to style the background color of the `Toolbar`.
...
Since I used webforJ to create this project, I used a Java loop to generate repeatable filler text that only took up space in the UI, not in my project's code. Also, even though it was use of more code lines, I put the content into a [`FlexLayout`](/docs/components/flex-layout) to make the project look more presentable.
...
## Adding the interactable aspects

### Composite component

Since all the UI needs to go somewhere, I decided to make my composite component another `FlexLayout`. My composite component also included the scrollbar, so anything larger than the screen will overflow.
...
### Updating the `PrograssBar` value
...
The following block of code is the full project of my reading position indicator, made within 100 lines of code. Even when I did need to use JavaScript, it was minimal and contained within my composite component. I hope my project gives you some inspiration for a webforJ-focused solutions for your next project.
