...
## Frontend bundler {#frontend-bundler}
...
## `AppNav` pinning and search {#appnav-pinning-and-search}
...
:::tip Learn More
See the [`AppNav`](/docs/components/appnav) documentation for pin events, custom persistence, and driving search from your own input.
:::
...
## Nested bean binding {#nested-bean-binding}
...
### App icon and favicon badges {#app-icon-and-favicon-badges}
...
## Deprecation: `<html>` in `setText` {#deprecation-html-in-settext}
...
The [webforJ rewrite recipes](/docs/upgrading/automated-upgrades) take care of most of this, rewriting an `<html>` wrapped `setText` literal to `setHtml` wherever the component supports it.

:::tip Learn More
See the [Automated Upgrades](/docs/upgrading/automated-upgrades) guide for the OpenRewrite recipes that migrate these calls for you.
:::
