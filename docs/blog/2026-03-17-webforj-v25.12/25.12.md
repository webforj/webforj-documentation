...
## Table column groups and cell renderers {#table-column-groups-and-cell-renderers}

The `Table` component picks up two major new features in `25.12`: [**column groups**](/docs/components/table/table_column_groups) for organizing complex headers, and a [**library of built-in cell renderers**](/docs/components/table/rendering) that eliminates the need for custom rendering boilerplate.
...
### Built-in cell renderers {#built-in-cell-renderers}
...
:::info Learn More
See the [Table Rendering](/docs/components/table/rendering) documentation for the full renderer catalog, conditional rendering, composites, and custom renderers.
:::

## Kotlin DSL {#kotlin-dsl}

For Kotlin developers, webforJ `25.12` introduces a [**Kotlin DSL module**](/docs/integrations/kotlin-dsl/overview) that brings a declarative, type-safe way to build UIs. The difference speaks for itself:
...
Getting started is lightweight: add two Maven dependencies, configure the Kotlin plugin, and you're writing DSL code alongside your existing Java files. No separate toolchain needed. Alternatively, skip the setup entirely and clone the [Kotlin Starter](https://github.com/webforj/webforj-kotlin-starter) to hit the ground running.

:::info Learn More
See the [Kotlin DSL](/docs/integrations/kotlin-dsl/overview) documentation for setup, usage patterns, and extensibility.
:::

## Built-in translation system {#built-in-translation-system}

Going global with your app just got a lot simpler. webforJ `25.12` ships a [**built-in translation system**](/docs/advanced/i18n-localization) that handles the heavy lifting of internationalization. Define your translations in standard Java `ResourceBundle` property files, and look them up anywhere with the new `t()` method.
...
First up, the [`Accordion`](/docs/components/accordion): a vertically stacked set of collapsible panels that keeps your layouts clean and your users focused. Each panel has a clickable header that toggles its content, and when multiple `AccordionPanel` components are grouped inside an `Accordion` group, opening one automatically collapses the others so only one section is visible at a time.
...
### Deprecation of unsupported Webswing bootstrap options {#deprecation-of-unsupported-webswing-bootstrap-options}
...
The [built-with-webforJ repository](https://github.com/webforj/built-with-webforj) continues to grow with new reference projects. This release adds:

### [Bookstore](https://github.com/webforj/built-with-webforj/tree/main/webforj-bookstore) {#bookstore}
...
<img src="https://cdn.webforj.com/webforj-documentation/blogs/webforj-v25.12/bookstore.png" alt="Bookstore" style={{width: '100%', borderRadius: '20px', border: '1px solid var(--ifm-color-emphasis-300)'}} />
