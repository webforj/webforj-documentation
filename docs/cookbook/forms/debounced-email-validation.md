...
Wire `onModify` to a `Debouncer` so the regular expression validation only runs after the user pauses typing. `setInvalid` plus `setInvalidMessage` toggle the field's built-in error state.
