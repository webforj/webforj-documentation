...
A `Button` component is a fundamental user interface element used in application development to create interactive elements that trigger actions or events when clicked or activated. It serves as a clickable element that users can interact with to perform various actions within a web application or website. 
...
1. **Form Submission**: Buttons are often used to submit form data. For example, in a web application, you can use:
...
1. <JavadocLink type="engine" location="org/dwcj/component/button/Button" code='true' suffix='#<init>()'>Button()</JavadocLink>: Creates an empty `Button` without any text inside the component.
2. <JavadocLink type="engine" location="org/dwcj/component/button/Button" code='true' suffix='#<init>(java.lang.String)'>Button(String text)</JavadocLink>: Creates a `Button` with text inside the component.
...
To read more about customizing, configuring and styling icons, read [**this page**](../components/related-components/BBjIcon).
...
Button components, like many others, can be disabled to convey to a user that a certain action is not yet or is no longer available. A disabled button will increase the opacity of the button, and is available for all button themes and expanses.
...
| Events | Description |
|:-:|-|
|[`BlurEvent`](../components/events/BlurEvent)|An event that is triggered when an element loses focus.|
...
|[`FocusEvent`](../components/events/FocusEvent)|An event that is triggered when an element gains focus, opposite of a blur event. |
|[`MouseEnterEvent`](../components/events/MouseEnterEvent)|An event that is triggered when the mouse cursor enters the boundaries of an element. |
|[`MouseExitEvent`](../components/events/MouseExitEvent)|An event that is triggered when the mouse cursor exits the boundaries of an element. |
|[`RightMouseDownEvent`](../components/events/RightMouseDownEvent)|An event that is triggered when the user presses the right mouse button while the cursor is over an element.|
...
While there are many use cases for each of the various themes, some examples uses are:
  ...
  - **Default**: The default theme is appropriate for actions throughout an application that do not require special attention and that are generic, such as toggling a letting or confirming an action.
