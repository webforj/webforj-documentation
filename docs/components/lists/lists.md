...
There are three types of lists for use within your applications: [`ListBox`](./list-box), [`ChoiceBox`](./choice-box) and [`ComboBox`](./combo-box). These three components all display a list of key value items, and support methods to add, remove, select and manage the items within the list.
