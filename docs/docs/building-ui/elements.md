...
### Adding child components {#adding-child-components}
...
1. **`remove(Component... components)`**: This method takes one or more components and removes them as child components.

2. **`removeAll()`**: This method removes all child components from the `Element`.
