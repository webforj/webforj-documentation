...
The `TabbedPane` class has two constituent parts: a `Tab` that's displayed in a specified location, and a component to be displayed. This can be a single component, or a [`Composite`](/docs/building-ui/composing-components) component, allowing for the display of more complex components within a tab's content section.
