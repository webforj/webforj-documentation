...
| **`webforj.devtools.craftforj.enabled`**&nbsp;<DocChip chip='since' label='26.02' /> | Boolean | **Development environments only.** Turns on [craftforJ](../craftforj/overview.md), the development environment that inspects the running app, edits component properties, and writes changes back to Java source. Requires `webforj.debug` to be enabled as well. Neither property is sufficient on its own. | `false` |
| **`webforj.devtools.craftforj.hosts-allowed`**&nbsp;<DocChip chip='since' label='26.02' /> | List | Client addresses allowed to reach craftforJ beyond the machine running the app. By default, only a browser on that machine can reach it. An entry ending in `*` matches a prefix, and a single `*` removes the restriction. See [craftforJ security](../craftforj/security.md). | loopback only |
| **`webforj.devtools.craftforj.project-root`**&nbsp;<DocChip chip='since' label='26.02' /> | String | The directory craftforJ looks in for your sources, for cases where it can't determine that from how the app was started. | detected |
| **`webforj.devtools.craftforj.source-changes`**&nbsp;<DocChip chip='since' label='26.02' /> | Boolean | Whether craftforJ may write property changes and route access rules into your Java source. | `true` |
| **`webforj.devtools.craftforj.stylesheet-changes`**&nbsp;<DocChip chip='since' label='26.02' /> | Boolean | Whether craftforJ may save themes and styles into your app stylesheet. | `true` |
| **`webforj.devtools.craftforj.ai.enabled`**&nbsp;<DocChip chip='since' label='26.02' /> | Boolean | Whether the [craftforJ AI assistant](../craftforj/ai.md) is available. | `true` |
| **`webforj.devtools.craftforj.ai.freeform-changes`**&nbsp;<DocChip chip='since' label='26.02' /> | Boolean | Whether the assistant may write Java of its own rather than only changing properties. Every edit must still compile and still needs your approval. | `true` |
...
| **`webforj.i18n.supported-locales`**&nbsp;<DocChip chip='since' label='25.12' /> | List | List of supported locales as BCP 47 language tags (e.g., `"en"`, `"en-US"`, `"fr"`, `"de-DE"`). When auto-detect is enabled, the browser's preferred locales are matched against this list. The first locale in the list is used as the default fallback. See [Translation](../advanced/i18n-localization.md). | `[]` |
| **`webforj.i18n.auto-detect`**&nbsp;<DocChip chip='since' label='25.12' /> | Boolean | When `true`, the application locale is automatically set from the browser's preferred language on startup. The locale is resolved by matching the browser's preferred locales against the `supported-locales` list. When `false` or when `supported-locales` is empty, the application uses `webforj.locale`. See [Translation](../advanced/i18n-localization.md). | `false` |
