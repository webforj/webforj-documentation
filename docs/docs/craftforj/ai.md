...
Full validation needs a JDK. On a JRE, craftforJ falls back to parsing the code, marks the edit as unverified, and instructs the agent to say so rather than presenting it as checked.
...
![The assistant asking before it acts, inline in the transcript](/img/craftforj/ai/approval-prompt.png#rounded-border)
...
![The model picker showing what distinguishes the available models](/img/craftforj/ai/model-picker.png#rounded-border)
...
Conversations are kept per app, and the agent can look back over earlier ones when a question refers to work you did before. When a conversation outgrows the model's context, craftforJ summarizes the older messages so the work continues rather than failing, and notes in the chat that it did.
