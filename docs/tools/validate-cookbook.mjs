...
/** Full set of recognized tag values - must match the list in recipe-template.mdx. */
...
/** Returns true when the Markdown content contains at least one fenced code block. */
...
async function getChangedFiles() {
  try {
    ...
    return output
      .split('\n')
      .map((f) => f.trim())
      .filter((f) => f.startsWith('docs/cookbook/') && f.match(/\.mdx?$/))
      .filter((f) => !f.endsWith('index.mdx')
        && !f.endsWith('recipe-template.md')
        && !f.endsWith('recipe-template.mdx'))
