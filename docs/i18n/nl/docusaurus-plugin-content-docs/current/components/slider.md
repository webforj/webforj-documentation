...
De `Slider`-component biedt gebruikers de mogelijkheid om een numerieke waarde te selecteren door een knoop langs een spoor tussen een minimum- en maximumgrens te slepen. Stapintervallen, tickmarks en labels kunnen worden geconfigureerd om de selectie te begeleiden.

<!-- INTRO_END -->

## Basisprincipes {#basics}

De `Slider` is ontworpen om direct uit de doos te werken, zonder extra opstelling om effectief te functioneren. Standaard beslaat het een bereik van 0 tot 100 met een startwaarde van 50, waardoor het ideaal is voor snelle integratie in elke app. Voor meer specifieke gebruikstoepassingen kan de `Slider` worden aangepast met eigenschappen zoals oriëntatie, tickmarks, labels en tooltips.
...
### De waarde instellen en ophalen {#setting-and-getting-the-value}
...
Deze intervallen zijn gelijkmatig verdeeld langs het slider-spoor, met de afstand afhankelijk van de afmetingen van de `Slider`.

Hieronder is een voorbeeld van het maken van een `Slider` met een aangepast bereik:
...
De `Slider`-component biedt flexibele tick-configuratie, waarmee je kunt aanpassen hoe tickmarks worden weergegeven en hoe de knop van de slider ermee omgaat. Dit omvat het aanpassen van de ruimte tussen grote en kleine ticks, het tonen/verbergen van tickmarks en het inschakelen van het vastklikken aan tickmarks voor nauwkeurige gebruikersinvoer.

### Ruimte tussen grote en kleine ticks {#major-and-minor-tick-spacing}

Je kunt de ruimte voor grote en kleine tickmarks definiëren, wat bepaalt hoe vaak ze op het slider-spoor verschijnen:

- Grote ticks zijn groter en vaak gelabeld om sleutelwaarden weer te geven.
- Kleine ticks zijn kleiner en verschijnen tussen grote ticks om fijnere intervallen te bieden.

Stel de tickruimte in met de volgende methoden `setMajorTickSpacing()` en `setMinorTickSpacing()`:
```java
...
Je kunt de zichtbaarheid van tickmarks om schakelen met de methode `setTicksVisible()`. Standaard zijn tickmarks verborgen.
...
Om ervoor te zorgen dat de knop van de `Slider` uitgelijnd is met de dichtstbijzijnde tickmark tijdens de gebruikersinteractie, schakel je vastklikken in met de methode `setSnapToTicks()`:

```java
slider.setSnapToTicks(true); // Inschakelen van vastklikken
```
...
De `Slider`-component ondersteunt twee oriëntaties: horizontaal (standaard) en verticaal. Je kunt de oriëntatie aanpassen aan jouw UI-indeling en app-vereisten.

Naast oriëntatie kan de `Slider` ook worden omgekeerd. Standaard:

- Een horizontale `Slider` gaat van minimaal (links) naar maximaal (rechts).
- Een verticale `Slider` gaat van minimaal (onder) naar maximaal (boven).
...
De `Slider`-component ondersteunt labels op tickmarks om gebruikers te helpen de waarden gemakkelijker te interpreteren. Je kunt standaard numerieke labels gebruiken of aangepaste labels opgeven, en je kunt hun zichtbaarheid indien nodig beheren.

### Standaardlabels {#default-labels}

Standaard kan de slider numerieke labels weergeven bij grote tickmarks. Deze waarden worden bepaald door de instelling `setMajorTickSpacing()`. Om standaardlabels in te schakelen, gebruik:
...
Je kunt de standaard numerieke labels vervangen door aangepaste tekst met behulp van de methode `setLabels()`. Dit is nuttig wanneer je meer betekenisvolle waarden wilt weergeven (bijv. temperatuur, valuta of categorieën).
...
### Zichtbaarheid van labels beheren {#toggling-label-visibility}

Of je nu standaard- of aangepaste labels gebruikt, je kunt hun zichtbaarheid beheren met `setLabelsVisible(true)` of ze verbergen met `setLabelsVisible(false)`.
...
Tooltips kunnen ook worden geconfigureerd om alleen te verschijnen wanneer de gebruiker met de `Slider` interactie heeft. Gebruik de methode `setTooltipVisibleOnSlideOnly()` om dit gedrag in te schakelen. Dit is vooral nuttig om visuele rommel te verminderen terwijl je nog steeds nuttige feedback biedt tijdens interactie.

Hier is een voorbeeld van een volledig geconfigureerde `Slider` met tooltips:


### Personalisatie van tooltips {#tooltip-customization}

Standaard toont de `Slider` een tooltip met zijn huidige waarde. Als je deze tekst wilt aanpassen, gebruik je de methode `setTooltipText()`. Dit is nuttig wanneer je wilt dat de tooltip statische of beschrijvende tekst toont in plaats van de live waarde.

Je kunt ook een JavaScript-expressie gebruiken om de tooltip dynamisch te formatteren. Als je expressie het `return`-trefwoord bevat, wordt deze zoals deze is gebruikt. Zo niet, dan wordt deze automatisch omgeven door `return` en `;` om een geldige functie te vormen. Bijvoorbeeld:
...
Hieronder worden sliders weergegeven met elk van de ondersteunde thema's toegepast:
