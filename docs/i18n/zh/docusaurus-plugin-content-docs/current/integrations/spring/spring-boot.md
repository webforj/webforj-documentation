...
---
Spring Boot 是构建 Java 应用的热门选择，提供了依赖注入、自动配置和嵌入式服务器模型。使用 Spring Boot 与 webforJ 时，您可以通过构造函数注入将服务、存储库和其他 Spring 管理的 bean 直接注入到您的 UI 组件中。

当使用 Spring Boot 与 webforJ 时，您的应用作为可执行 JAR 运行，配有嵌入式 Tomcat 服务器，而不是将 WAR 文件部署到外部应用服务器。该打包模型简化了部署，并与云原生部署实践保持一致。 webforJ 的组件模型和路由与 Spring 的应用上下文协同工作，以管理依赖关系和配置。

## 创建一个 Spring Boot 应用 {#create-a-spring-boot-app}

有两种选项可以使用 Spring Boot 创建新的 webforJ 应用：使用图形化的 startforJ 工具或 Maven 命令行。

<!-- vale off -->
### 选项 1：使用 startforJ {#option-1-using-startforj}
<!-- vale on -->

创建新的 webforJ 应用的最简单方法是 [startforJ](https://docs.webforj.com/startforj)，它根据选择的 webforJ 原型生成一个最小的起始项目。该起始项目包括所有必需的依赖项、配置文件和预制布局，以便您可以立即开始构建。
...
- 主题颜色和图标
- 原型
- **Flavor** - 选择 **webforJ Spring** 以创建一个 Spring Boot 项目
...
如果您更喜欢使用命令行，可以直接通过官方的 webforJ 原型生成一个 Spring Boot webforJ 项目：
...
如果您有一个现有的 webforJ 应用，可以通过修改项目配置来添加 Spring Boot。这个过程包括更新您的 Maven 配置、添加 Spring 依赖项和转换您的主应用类。

:::info[仅适用于现有项目]
如果您是从头开始创建新项目，请跳过此部分。
:::
...
如果您已经有一个父 POM，您需要导入 Spring Boot 材料清单 (BOM) 而不是：
...
`webforj-spring-devtools` 依赖项扩展了 Spring DevTools，提供自动浏览器刷新。 当您在 IDE 中保存更改时，浏览器会自动重新加载，无需手动干预。 请参阅 [Spring DevTools](/docs/configuration/deploy-reload/spring-devtools) 指南以获取配置详情。
:::
...
应用将默认在 8080 端口启动嵌入式 Tomcat 服务器。您现有的 webforJ 视图和路由依旧正常工作，但现在您可以注入 Spring bean 并使用 Spring 功能。

## 配置

使用 `src/main/resources` 中的 `application.properties` 文件配置您的应用。 
请参阅 [属性配置](/docs/configuration/properties) 以获取有关 webforJ 配置属性的信息。
...
| **`webforj.exclude-urls`** | 列表 | 当映射到根时，不应由 webforJ 处理的 URL 模式。 当 webforJ 映射到根上下文 (`/*`) 时，这些 URL 模式将被排除在 webforJ 处理之外，并可以由 Spring MVC 控制器处理。 这允许 REST 端点和其他 Spring MVC 映射与 webforJ 路由共存。 | `[]` |

### 配置差异 {#configuration-differences}

切换到 Spring Boot 时，多项配置方面会发生变化：
