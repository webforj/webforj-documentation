...
`Repository`模式在webforJ中提供了一种标准化的方式来管理和查询实体集合。它作为UI组件和数据之间的抽象层，使得在维护一致性行为的同时，轻松处理不同的数据源。
...
<JavadocLink type="data" location="com/webforj/data/repository/CollectionRepository" code="true">CollectionRepository</JavadocLink>是最常见的实现，包装任何Java集合：
...
`Repository`充当您的数据和UI组件之间的桥梁。当数据发生变化时，您通过`commit()`方法通知仓库：
...
commit方法有两个签名：
- `commit()` - 告诉仓库刷新所有内容。它会触发一个`RepositoryCommitEvent`，包含所有当前数据
- `commit(entity)` - 目标是特定实体。仓库通过其关键字查找该实体，仅更新受影响的UI元素

:::important 提交单个实体
这一区分对于性能很重要。当您在一个1000行的表中更新一个字段时，`commit(entity)`只会更新该单元格，而`commit()`将刷新所有行。
:::

## 数据过滤 {#filtering-data}

仓库的过滤器控制传递到连接组件的数据。您的基础集合保持不变，因为过滤器充当透镜：
...
过滤器在您更改之前将持续有效。添加到集合中的新项目会自动根据当前过滤器进行测试。


## 处理实体键 {#working-with-entity-keys}

仓库需要唯一标识实体以支持`find()`和`commit(entity)`等操作。定义实体如何被识别有两种方法：
...
customer.ifPresent(c -> {
  ...
  repository.commit(c); // 只更新该客户的行
});
```

### 使用自定义键提供程序 <DocChip chip='since' label='25.10' /> {#using-custom-key-provider}

对于您无法或不想实现`HasEntityKey`的实体（如JPA实体），请使用`setKeyProvider()`：
...
- 处理具有`@Id`字段的JPA实体时
- 您无法修改实体类时
- 您需要为不同的仓库使用不同的键策略时，推荐使用`setKeyProvider()`。

当：
- 您控制实体类时
- 键提取逻辑复杂时
- 您希望实体定义自己的身份时，请使用`HasEntityKey`。
