...
2. **组件线程亲和力**：用户界面组件维护的状态不是线程安全的
3. **事件分发**：所有用户界面事件都在单个线程上按顺序处理
...
这两种方法都返回一个 <JavadocLink type="foundation" location="com/webforj/PendingResult" code='true'>PendingResult</JavadocLink>，它跟踪任务完成情况并提供对结果或发生的任何异常的访问。
...
在 `Environment` 上下文之外创建的线程无法使用 `runLater()`，并会抛出 `IllegalStateException`：
...
通过这种同步行为，事件处理程序中的用户界面更新立即应用，并且不会产生任何不必要的排队开销。

### 从后台线程 {#from-background-threads}

当从后台线程调用时，任务 **排队以进行异步执行**：
...
webforJ 以 **严格的 FIFO 顺序** 处理来自后台线程提交的任务，在多个线程并发提交时仍然保持操作顺序。通过这种排序保证，用户界面的更新将以提交的确切顺序应用。如果线程 A 提交任务 1，然后线程 B 提交任务 2，则任务 1 始终会在用户界面线程上执行，然后才是任务 2。按 FIFO 顺序处理任务可以防止用户界面中的不一致性。

## 任务取消 {#task-cancellation}

`Environment.runLater()` 返回的 <JavadocLink type="foundation" location="com/webforj/PendingResult" code='true'>PendingResult</JavadocLink> 支持取消，允许您防止排队任务执行。通过取消待处理任务，您可以避免内存泄漏，并防止长时间运行的操作在不再需要时更新用户界面。
...
在进行需要频繁用户界面更新的长时间运行的操作时，跟踪所有待处理结果：
...
2. **内存泄漏预防**：始终在组件生命周期方法中跟踪和取消 `PendingResult` 对象。排队的 lambda 捕获对用户界面组件的引用，如果不取消，将阻止垃圾收集。

3. **FIFO 执行**：所有任务以严格的 FIFO 顺序执行，而不考虑重要性。没有优先级系统。

4. **取消限制**：取消仅阻止排队任务的执行。已经在执行的任务将正常完成。

## 完整案例研究：`LongTaskView` {#complete-case-study-longtaskview}

以下是一个完整且生产就绪的实现，展示了异步用户界面更新的所有最佳实践：
...
public class LongTaskView extends Composite<FlexLayout> {
  ...
  private H2 titleLabel = new H2("后台用户界面更新演示");
  private Paragraph descriptionPara = new Paragraph(
      "本演示展示了如何通过 Environment.runLater() 安全地从后台线程更新用户界面。" +
          "单击“开始长任务”以运行一个 10 秒的后台计算，更新用户界面进度。" +
          "“测试用户界面”按钮证明在后台操作期间用户界面保持响应。");
  ...
  private Button testButton = new Button("测试用户界面 - 点击我！");
  private Paragraph footerPara = new Paragraph(
      "注意：任务可以随时取消，演示了后台线程和排队用户界面更新的适当清理。");
  ...
  public LongTaskView() {
    ...
    testButton.onClick(e -> {
      ...
      showToast("点击 #" + count + " - 用户界面响应顺畅！", Theme.GRAY);
    });
...
#### 2. 跟踪待处理的更新 {#2-tracking-pending-updates}
```java
private final List<PendingResult<?>> pendingUIUpdates = new ArrayList<>();
```
每次 `Environment.runLater()` 调用都会被跟踪，以实现：
- 当用户点击取消时的取消
- 在 `onDestroy()` 中防止内存泄漏
- 组件生命周期中的适当清理
...
#### 5. 用户界面响应性测试 {#5-ui-responsiveness-testing}
```java
testButton.onClick(e -> {
  int count = clickCount.incrementAndGet();
  showToast("点击 #" + count + " - 用户界面响应顺畅！", Theme.GRAY);
