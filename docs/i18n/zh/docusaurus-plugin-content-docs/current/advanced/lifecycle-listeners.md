...
| `onDidRun`            | 在 `app.run()` 成功完成后                         | 启动后台任务，记录成功启动信息                      |
| `onWillTerminate`     | 在应用终止之前                                     | 保存状态，为关闭做准备                              |
| `onDidTerminate`      | 在应用终止之后                                     | 清理资源，进行最终记录                              |
...
### 与 App 钩子的执行流程 {#execution-flow-with-app-hooks}
