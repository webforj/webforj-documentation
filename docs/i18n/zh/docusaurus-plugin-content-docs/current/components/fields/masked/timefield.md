...
`MaskedTimeField` 是一个文本输入框，允许用户以 **数字** 形式输入时间，并在字段失去焦点时根据定义的掩码自动格式化输入。掩码指定了期望的时间格式，以指导输入和显示。该组件支持灵活的解析、验证、本地化和数值恢复，以便于一致的时间处理。

<!-- INTRO_END -->

:::tip 寻找日期输入？
`MaskedTimeField` 专为 **仅时间** 输入而构建。如果您需要处理 **日期** 的组件，并具有类似的基于掩码的格式，请查看 [`MaskedDateField`](/docs/components/fields/masked/datefield)。
:::
...
:::tip 以编程方式应用掩码
要在字段之外使用相同的掩码语法格式化或解析时间，请使用 [`MaskDecorator`](/docs/advanced/mask-decorator) 实用类。
:::
...
`MaskedTimeField` 通过设置适当的区域设置支持本地化。这确保时间输入和输出符合区域惯例。

```java
field.setLocale(Locale.GERMANY);
```

这会影响上午/下午指示符的显示方式、分隔符的处理方式以及数值的解析方式。

## 解析逻辑 {#parsing-logic}

`MaskedTimeField` 根据定义的时间掩码解析用户输入。它接受完整和简略的数字输入，并可有或没有分隔符，从而允许灵活输入，同时确保有效的时间。
解析行为取决于掩码定义的格式顺序（例如，`%Hz:%mz` 用于小时/分钟）。此格式确定数字序列的解释方式。
...
这两个方法接受类型为 [`java.time.LocalTime`](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/LocalTime.html) 的值。超出定义范围的输入将被视为无效。

## 恢复值 {#restoring-the-value}

`MaskedTimeField` 包含一个还原功能，可以将字段的值重置为预定义或原始状态。这在撤消更改或恢复到默认时间时非常有用。
...
- **通过键盘**，按 <kbd>ESC</kbd>（这是默认的还原键，除非被事件监听器覆盖）
...
此模式确保仅与 `HH:mm` 格式（两个数字，冒号，两个数字）匹配的值被视为有效。

:::tip 正则表达式格式
该模式必须遵循 JavaScript 正则表达式语法，详细文档可以参见 [这里](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_expressions)。
:::

:::warning 输入处理说明
该字段尝试基于当前掩码解析和格式化数字时间输入。然而，用户仍然可以手动输入不匹配预期格式的值。如果输入在语法上有效但在语义上不正确或不可解析（例如 `99:99`），它可能会通过模式检查但在逻辑验证中失败。
即使设置了正则表达式模式，您仍然应该在应用逻辑中验证输入值，以确保时间不仅格式正确而且有意义。
:::

## 时间选择器 {#time-picker}

`MaskedTimeField` 包含一个内置的时间选择器，允许用户可视化选择时间，而不是键入输入。这提高了对不太技术用户的可用性，或在需要精确输入的情况下。
...
:::tip 通过选择器强制选择
为了确保用户只能使用选择器选择时间（而不是手动输入），请结合以下两个设置：
...
此设置确保所有时间输入通过选择器 UI 进行，这在需要严格格式控制和消除输入解析问题时非常有用。
:::
...
您可以使用 `setStep()` 定义选择器中可选择时间之间的间隔。这允许您控制时间选项的粒度——非常适合于分段如 15 分钟的调度场景。
...
`MaskedTimeFieldSpinner` 通过添加旋转器控件来扩展 `MaskedTimeField`，使用户能够使用箭头键或 UI 按钮增加或减少时间。它提供了一种更有引导性的交互样式，尤其适用于桌面风格的应用程序。
