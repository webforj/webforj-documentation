...
`Tree` 组件将数据组织为节点的层次结构。每个节点持有一个唯一的键和一个标签。节点通过连接形成父子关系。您可以展开或折叠节点以显示或隐藏其子节点。图标可以清晰地表明您正在查看哪种类型的节点以及该节点是否已被选中。选择支持一次选择一个或多个节点。
...
树中的每个数据项都被封装在一个 `TreeNode` 中。这个对象持有键、文本标签，并链接到其父节点和子节点。根节点是特殊的：它存在于每颗树中，但并不可见。它作为所有顶级节点的容器，使树结构在内部更易于管理。

由于节点保持对其父节点和子节点的引用，遍历树是直接的。无论您是想向上、向下移动，还是通过键查找特定节点，这些连接都是始终可访问的。

### 节点的创建和管理 {#node-creation-and-management}

节点是通过简单的工厂方法创建的，可以提供一个键和文本，或仅提供文本（文本同时作为键）。这保证了每个节点都是有效的并且可以独特地识别。
...
:::info 仅单一父节点
尝试将同一节点分配给多个父节点将导致抛出异常。此保护措施确保树保持适当的层次结构，防止节点有多个父节点，这会破坏结构的完整性并导致意外行为。
:::
...
要删除特定的子节点，使用 `remove(TreeNode child)`。这将子节点与其父节点断开，并从树结构中移除。它还会清除父引用。

如果您想要清除节点的所有子节点，调用 `removeAll()`。这将移除每个子节点，清除其父引用，并清空子节点列表。

每个节点支持通过 `setUserData(Object key, Object data)` 存储附加信息在服务器端。这使您可以将任意元数据或引用与节点关联，而无需将这些数据暴露给客户端或用户界面。
...
图标提供有关节点代表的内容和它们状态的视觉提示。它们通过一眼区分节点类型和选择状态来提高可读性。`Tree` 组件支持全局设置默认图标，为节点自定义图标和切换图标可见性。

### 全局图标 {#global-icons}

树允许您为折叠组、展开组、叶子节点和选中叶子节点设置默认图标。
...
您可以通过为单个节点分配图标来覆盖全局默认值。这在某些节点代表不同概念时，例如“项目”文件夹或特殊文件时非常有用。
...
有时，您可能希望隐藏组或叶子的图标以减少杂乱。组件允许您全局切换这些类别的可见性，以简化树的外观，而不失去结构。
...
## 节点扩展和折叠 {#node-expansion-and-collapse}

节点可以展开或折叠，以控制树的可见部分。这允许专注于相关部分，并支持懒加载或动态数据更新的场景。

### 展开和折叠操作 {#expand-and-collapse-operations}

树支持通过其键或直接引用展开和折叠单个节点。您还可以一次展开或折叠节点的所有后代。

这些操作允许您控制可见的树的大小，并支持数据的懒加载或集中于感兴趣的区域。
...
:::info 折叠根节点
根节点锚定了树，但保持隐藏。折叠根节点通常会隐藏一切，使得树看起来为空。为了避免这种情况，折叠根节点实际上只折叠所有它的子节点，但内部保持根节点展开，确保树仍然正确显示其内容。
:::

### 懒加载节点 {#lazy-loading-nodes}

树支持通过响应展开事件懒加载节点子节点。当用户展开节点时，您的应用可以动态加载或生成该节点的子节点。这通过按需仅加载可见部分的树来提高性能。

使用 `onExpand` 事件来检测节点何时被展开。在处理程序内部，检查节点的子节点是否是占位符（例如，旋转器或空节点），并在加载后用实际数据替换它们。
...
选择控制用户选择哪些节点。`Tree` 组件支持灵活的模式和 API，用于选择、取消选择和查询节点。

### 选择模式 {#selection-modes}

您可以选择树是否允许一次选择一个节点或同时选择多个节点。从多个选择切换到单个选择会自动取消选择除第一个选中的节点以外的所有节点。
...
* **桌面（鼠标和键盘）：** 用户按住 **Ctrl** 键（或在 macOS 上按 **Cmd** 键）并点击节点，以将它们添加到当前选择或从中移除。这样可以选择多个独立节点而不取消选择其他节点。
* **移动和触摸设备：** 由于没有可用的修饰键，用户只需点击节点即可选择或取消选择它们。每次点击都会切换该节点的选择状态，使通过简单的点击轻松实现多重选择。
:::

### 选择和取消选择 {#selecting-and-deselecting}

可以通过引用、键、单独或批量选择或取消选择节点。您还可以在一次调用中选择或取消选择节点的所有子节点。
