...
`Toolbar` 以易于访问和一致的布局组织基本组件。默认情况下，它占据其父元素的整个宽度，并提供四个放置区域或 _插槽_ 来组织组件：
...
要了解更多关于在 `AppLayout` 中实现工具栏的信息，请参见 [粘性工具栏](./app-layout#sticky-toolbars) 和 [移动导航布局](./app-layout#mobile-navigation-layout)。
...
`Toolbar` 组件包含 <JavadocLink type="foundation" location="com/webforj/component/Theme">七种内置主题</JavadocLink> 以实现快速视觉自定义：
