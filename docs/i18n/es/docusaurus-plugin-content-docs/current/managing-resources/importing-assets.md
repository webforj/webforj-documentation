...
:::tip El bundler es la opción predeterminada para npm y frameworks
Las anotaciones de activos adjuntan un script o una hoja de estilo que ya tienes, sin necesidad de un paso de construcción. Para incorporar paquetes de npm, un framework de componentes como React, o un lenguaje de hojas de estilo como SCSS, utiliza el [frontend bundler](/docs/managing-resources/bundler/overview). Es el camino predeterminado para ese trabajo, y hace todo lo que hacen las anotaciones.
:::
...
| `attributes` | Object  | Un conjunto de <JavadocLink type="foundation" location="com/webforj/annotation/Attribute" code='true'>atributos</JavadocLink> que aplicar al script. | `{}`           |
...
:::info
Los archivos se cargan solo cuando el componente que declara la anotación está adjunto a un contenedor. Si varios componentes cargan el mismo archivo, el archivo solo se inyecta una vez.
:::

## Inyectando JavaScript {#injecting-javascript}

En algunos casos, puede que desees inyectar código JavaScript directamente en el DOM en lugar de proporcionar una ruta de JavaScript. La anotación `InlineJavaScript` te permite inyectar contenido de JavaScript.
...
| `id`         | `String` | Un ID de recurso único para garantizar una sola inyección               | `""`           |
...
La inclusión declarativa de CSS se admite a través de la anotación `@StyleSheet`, lo que permite la carga automática de dependencias. La anotación se puede aplicar tanto a nivel de componente como a nivel de aplicación.
...
| `attributes` | Object  | Atributos para aplicar a la hoja de estilo                                     | `{}`           |
...
| `attributes` | Object  | Un conjunto de [atributos](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/style) que aplicar al elemento de estilo. | `{}`           |
...
:::tip 
Para una mejor resaltación de sintaxis al escribir CSS en línea para tus componentes, puedes usar la extensión de VS Code de webforJ: [Java HTML CSS Syntax Highlighting](https://marketplace.visualstudio.com/items?itemName=BEU.vscode-java-html).
:::

## Activos dinámicos en tiempo de ejecución {#dynamic-assets-at-runtime}

La gestión de recursos dinámicos es posible a través de la inyección programática de JavaScript y CSS en tiempo de ejecución. Puedes cargar o inyectar recursos según el contexto en tiempo de ejecución.

### Cargando e inyectando JavaScript {#loading-and-injecting-javascript}

Carga o inyecta dinámicamente JavaScript en tiempo de ejecución utilizando la <JavadocLink type="foundation" location="com/webforj/Page" code='true'>API Page</JavadocLink>. Esto te permite cargar scripts desde URLs o inyectar scripts en línea directamente en el DOM.
...
| ------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `script`     | El URL o contenido del script en línea para inyectar. Los URLs que comienzan con `context://` se resuelven en la carpeta de recursos raíz de la aplicación. |
...
Carga o inyecta dinámicamente CSS en tiempo de ejecución utilizando la <JavadocLink type="foundation" location="com/webforj/Page" code='true'>API Page</JavadocLink>. Esto te permite cargar hojas de estilo desde URLs o inyectar estilos en línea directamente en el DOM.
...
| Parámetro    | Descripción                                                                                                                 |
| ------------ | --------------------------------------------------------------------------------------------------------------------------- |
| `stylesheet` | El URL o contenido de la hoja de estilo en línea para inyectar. Los URLs que comienzan con `context://` se resuelven en la carpeta de recursos raíz de la aplicación. |
| `top`        | Determina si la hoja de estilo debe inyectarse en la parte superior de la página.                                            |
| `attributes` | Un mapa de atributos para establecer para la hoja de estilo.                                                                  |
