...
El componente `GoogleChart` integra la biblioteca de [Google Charts](https://developers.google.com/chart) en webforJ, dándote acceso a tipos de gráficos como barras, líneas, tartas, geo, y más. Los gráficos se configuran con Java utilizando un tipo, un conjunto de datos y un mapa de opciones que controla la apariencia y el comportamiento.
...
Este ejemplo crea un gráfico geo que mapea los datos de ingresos a través de diferentes países, con colores personalizados, posicionamiento de leyenda y tamaño del área del gráfico:
...
El complemento `GoogleChart` ofrece una amplia gama de tipos de gráficos para satisfacer diversos requisitos de visualización de datos. Seleccionar el tipo de gráfico adecuado es esencial para comunicar de manera efectiva la historia de los datos. Consulta la galería a continuación para ejemplos de gráficos comunes que pueden ser utilizados en una aplicación webforJ.
...
El complemento `GoogleChart` permite una personalización extensa a través de una variedad de opciones. Estas opciones te permiten ajustar la apariencia y funcionalidad de tus gráficos para adaptarse a las necesidades de tu aplicación. Las opciones se pasan como un `Map<String, Object>` al método `setOptions()` del gráfico. 

Aquí tienes un ejemplo de cómo establecer las opciones de un gráfico:
...
### Configuración de datos básica {#basic-data-setup}
...
También puedes cargar datos y opciones desde archivos JSON usando Gson para una gestión más sencilla. Este enfoque ayuda a mantener tus datos y opciones organizados y fáciles de actualizar.
...
## Actualizando los aspectos visuales del gráfico {#updating-chart-visuals}
...
- **Para ajustes adaptativos**: Ajusta el diseño o tamaño del gráfico cuando cambian las dimensiones del contenedor, asegurando una visualización óptima en todos los dispositivos.
...
El método `getImageUri()` proporciona una manera de exportar tus Google Charts como imágenes PNG codificadas en base64. Este método es particularmente útil para compartir gráficos fuera del entorno web, incrustarlos en correos electrónicos o documentos, o simplemente para fines de archivo.

Llama a `getImageUri()` en tu instancia de gráfico después de que el gráfico ha sido completamente renderizado. Típicamente, este método se usa dentro de un "escuchador de eventos listo" para asegurar que el gráfico esté listo para la exportación:
...
El `GoogleChartSelectedEvent` se activa cada vez que un usuario selecciona un punto de datos o segmento en un componente Google Chart. Este evento permite la interacción con los datos seleccionados en el gráfico, proporcionando detalles sobre lo que se seleccionó. El evento se puede escuchar utilizando el método `addSelectedListener()` en la instancia de `GoogleChart`.
...
// Agregar el escuchador seleccionado al gráfico
...
### Payload {#payload}
El `GoogleChartSelectedEvent` proporciona acceso a los datos de selección, los cuales se pueden recuperar utilizando el método `getSelection()` en el objeto del gráfico. Este método devuelve una lista de objetos, donde cada objeto contiene las siguientes propiedades:
...
Para gráficos como gráficos circulares o de barras, generalmente solo se proporciona `row`, indicando el punto de datos seleccionado.
