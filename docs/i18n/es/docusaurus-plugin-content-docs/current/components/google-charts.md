...
El componente `GoogleChart` integra la biblioteca de [Google Charts](https://developers.google.com/chart) en webforJ, dándote acceso a tipos de gráficos como barras, líneas, pasteles, geo, y más. Los gráficos se configuran con Java utilizando un tipo, un conjunto de datos y un mapa de opciones que controla la apariencia y el comportamiento.
