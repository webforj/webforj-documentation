...
El `MaskedDateField` es un campo de texto que permite a los usuarios ingresar fechas como números y formatea automáticamente la entrada según una máscara definida cuando el campo pierde el foco. La máscara especifica el formato de fecha esperado, guiando tanto la entrada como la visualización. El componente admite un análisis flexible, validación, localización y restauración de valores para un manejo de fechas consistente y específico de la región.

<!-- INTRO_END -->

## Básicos {#basics}

:::tip ¿Buscas un campo de entrada de tiempo?
El `MaskedDateField` se centra exclusivamente en valores de **fecha**. Si necesitas un componente similar para ingresar y formatear **hora**, consulta el [`MaskedTimeField`](./timefield) en su lugar.
:::
...
El `MaskedDateField` admite múltiples formatos de fecha utilizados alrededor del mundo, que varían por el orden de día, mes y año. Los patrones comunes incluyen:
...
El `MaskedDateField` se adapta a los formatos de fecha regionales configurando la localidad adecuada. Esto asegura que las fechas se visualicen y analicen de una manera que coincida con las expectativas del usuario.
...
Para aplicar la localización, utiliza el método `setLocale()`. Este acepta un [`java.util.Locale`](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/Locale.html) y ajusta automáticamente tanto el formato como el análisis:
...
El `MaskedDateField` analiza la entrada del usuario basándose en la máscara de fecha definida. Acepta tanto entradas numéricas completas como abreviadas con o sin delimitadores, permitiendo una entrada flexible mientras asegura la validez de las fechas.
...
Por ejemplo, supongamos que hoy es `15 de septiembre de 2012`, así sería como se interpretarían varias entradas:

### Ejemplos de escenarios de análisis {#example-parsing-scenarios}

| Entrada                             | YMD (ISO)                                                                                                                                                                                          | MDY (US)                                                                            | DMY (EU)                                                                                                                   |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| <div align="center">`1`</div>        | Un solo dígito siempre se interpreta como un número de día dentro del mes actual, por lo que esto sería el 1 de septiembre de 2012.                                                                 | Lo mismo que YMD                                                                     | Lo mismo que YMD                                                                                                          |
| <div align="center">`12`</div>       | Dos dígitos siempre se interpretan como un número de día dentro del mes actual, por lo que esto sería el 12 de septiembre de 2012.                                                                 | Lo mismo que YMD                                                                     | Lo mismo que YMD                                                                                                          |
| <div align="center">`112`</div>      | Tres dígitos se interpretan como un número de mes de 1 dígito seguido de un número de día de 2 dígitos, por lo que esto sería el 12 de enero de 2012.                                              | Lo mismo que YMD                                                                     | Tres dígitos se interpretan como un número de día de 1 dígito seguido de un número de mes de 2 dígitos, por lo que esto sería el 1 de diciembre de 2012. |
| <div align="center">`1004`</div>     | Cuatro dígitos se interpretan como MMDD, por lo que esto sería el 4 de octubre de 2012.                                                                                                           | Lo mismo que YMD                                                                     | Cuatro dígitos se interpretan como DDMM, por lo que esto sería el 10 de abril de 2012.                                    |
...
Por defecto, el `MaskedDateField` solo acepta entradas numéricas para fechas. Sin embargo, puedes habilitar el **análisis de fechas textuales** para permitir a los usuarios introducir nombres de meses y días en su entrada. Esta función es particularmente útil para crear una entrada de fecha más natural.
...
### Sustitución de nombres de meses {#month-name-substitution}

Cuando se habilita el análisis textual, puedes usar modificadores especiales en tu máscara para aceptar nombres de meses en lugar de valores numéricos:

- **`%Ms`** - Acepta nombres de meses cortos (Ene, Feb, Mar, etc.)
- **`%Ml`** - Acepta nombres de meses largos (Enero, Febrero, Marzo, etc.)

Los nombres de meses pueden aparecer en cualquier posición dentro de la máscara, y el campo seguirá aceptando entradas numéricas como respaldo.
...
| `%Mz/%Dz/%Yz` | `09/01/25` | **Válido** - El respaldo numérico aún funciona |

:::info
Los 12 meses están soportados en ambos formatos, corto (Ene, Feb, Mar, Abr, May, Jun, Jul, Ago, Sep, Oct, Nov, Dic) y largo (Enero, Febrero, etc.).
:::
...
Al utilizar nombres de días de la semana (`%Ds` o `%Dl`), tu máscara **también debe incluir** `%Dz` o `%Dd` para especificar el número real del día. Sin un componente de día numérico, la entrada será inválida.
:::
...
Todos los 7 días de la semana están soportados en ambos formatos, corto (Lun, Mar, Mié, Jue, Vie, Sab, Dom) y largo (Lunes, Martes, etc.).

### Reglas adicionales de análisis {#additional-parsing-rules}

El análisis de fechas textuales incluye varias funciones útiles:

- **No es sensible a mayúsculas:** Entradas como `LUNES 09/01/25`, `lunes 09/01/25`, o `Lunes 09/01/25` funcionan de la misma manera.
- **Consciente de la localidad:** Los nombres de meses y días deben coincidir con la localidad configurada del campo. Por ejemplo, con una localidad francesa, utiliza `septembre`, no `September`. Los nombres en inglés no serán reconocidos a menos que la localidad esté configurada en inglés.
...
Ambos métodos aceptan valores de tipo [`java.time.LocalDate`](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/LocalDate.html). La entrada fuera del rango definido se considerará inválida.

## Restaurando el valor {#restoring-the-value}

El `MaskedDateField` incluye una función de restauración que reinicia el valor del campo a un estado predefinido u original. Esto es útil para revertir la entrada del usuario o restablecer a una fecha predeterminada.
...
Este patrón asegura que solo se consideren válidos los valores que coinciden con el formato `MM/DD/YYYY` (dos dígitos, barra, dos dígitos, barra, cuatro dígitos).
...
El `MaskedDateField` incluye un selector de calendario integrado que permite a los usuarios seleccionar una fecha visualmente, en lugar de escribirla. Esto mejora la usabilidad para usuarios menos técnicos o cuando se requiere entrada precisa.
...
Utiliza `setIconVisible()` para mostrar u ocultar el icono del calendario junto al campo:
...
Puedes configurar el selector para que se abra automáticamente cuando el usuario interactúa con el campo (por ejemplo, clic, presiona Enter o teclas de flecha):
...
Para asegurar que los usuarios solo puedan seleccionar una fecha mediante el selector de calendario (y no escribirla manualmente), combina las siguientes dos configuraciones:
...
Esta configuración garantiza que toda la entrada de fecha provenga de la interfaz del selector, lo que es útil cuando deseas un control estricto sobre el formato y eliminar problemas de análisis de entradas escritas.
:::
...
El `MaskedDateFieldSpinner` extiende [`MaskedDateField`](#basics) al añadir controles de spinner que permiten a los usuarios aumentar o disminuir la fecha usando teclas de flecha o botones de la interfaz. Proporciona un estilo de interacción más guiado, especialmente útil en aplicaciones de estilo de escritorio.
...
- **Paso de Fecha Interactivo:**
  Utiliza teclas de flecha o botones de giro para aumentar o disminuir el valor de la fecha.
...
- **Fronteras Mínimas/Máximas:**
  Hereda soporte para las fechas mínimas y máximas permitidas usando `setMin()` y `setMax()`.

- **Salida Formateada:**
  Totalmente compatible con las máscaras y configuraciones de localización del `MaskedDateField`.

### Ejemplo: Configurar el paso semanal {#example-configure-weekly-stepping}
