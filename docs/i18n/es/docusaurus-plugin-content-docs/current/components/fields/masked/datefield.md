...
El `MaskedDateField` se centra exclusivamente en los valores de **fecha**. Si necesitas un componente similar para ingresar y formatear **tiempo**, consulta el [`MaskedTimeField`](./timefield).
:::
...
Para formatear o analizar fechas con la misma sintaxis de máscara fuera de un campo, usa la clase utilitaria [`MaskDecorator`](/docs/advanced/mask-decorator).
:::
...
El `MaskedDateField` analiza la entrada del usuario según la máscara de fecha definida. Acepta tanto entradas numéricas completas como abreviadas con o sin delimitadores, lo que permite una entrada flexible mientras asegura fechas válidas.
...
## Estableciendo límites min/max {#setting-minmax-constraints}
...
### Ejemplo: Configurar el aumento semanal {#example-configure-weekly-stepping}
