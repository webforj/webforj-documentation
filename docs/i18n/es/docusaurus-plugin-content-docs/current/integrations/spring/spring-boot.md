...
- Color del tema y icono
- Arquetipo
- **Sabor** - Selecciona **webforJ Spring** para crear un proyecto Spring Boot

Con esta información, startforJ creará un proyecto básico a partir del arquetipo elegido configurado para Spring Boot.
...
- Dependencia de iniciador de Spring Boot de webforJ
...
Omitir esta sección si estás creando un nuevo proyecto desde cero. Esta guía supone **webforJ versión 25.11 o posterior**.
:::
...
2. Establece Spring Boot como el POM principal:
   ```xml title="pom.xml"
   <parent>
...
Si ya tienes un POM principal, necesitarás importar el Bill of Materials (BOM) de Spring Boot en su lugar:
...
:::info[Simplificación webforJ 25.11+]
A partir de **la versión 25.11 de webforJ**, el `webforj-spring-boot-starter` incluye todas las dependencias centrales de webforJ de forma transitiva. Ya no es necesario agregar explícitamente la dependencia `com.webforj:webforj`.

Para versiones **anterior a 25.11**, debes incluir ambas dependencias por separado.
:::
...
```xml title="pom.xml"
<dependencies>
    <!-- Agrega explícitamente la dependencia de webforJ -->
    <dependency>
        <groupId>com.webforj</groupId>
...
:::tip[DevTools de webforJ para refresco automático del navegador]
La dependencia `webforj-spring-devtools` extiende Spring DevTools con refresco automático del navegador. Cuando guardas cambios en tu IDE, el navegador se recarga automáticamente sin intervención manual. Consulta la guía de [Spring DevTools](/docs/configuration/deploy-reload/spring-devtools) para detalles de configuración.
:::
...
public class Application extends App {
    ...
    // Mantén tu método run() existente si lo tienes
    @Override
...
La anotación `@SpringBootApplication` habilita la auto-configuración de Spring y el escaneo de componentes. La anotación `@Routify` permanece igual, continuando escaneando tus paquetes de vista para las rutas.
...
# Nombre completo de la clase del punto de entrada de la aplicación
