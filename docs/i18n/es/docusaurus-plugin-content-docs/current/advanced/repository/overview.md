...
El patrón `Repositorio` en webforJ proporciona una forma estandarizada de gestionar y consultar colecciones de entidades. Actúa como una capa de abstracción entre tus componentes de UI y los datos, facilitando el trabajo con diferentes fuentes de datos mientras mantiene un comportamiento consistente.
...
- `commit(entity)` - Destina a una entidad específica. El repositorio encuentra esta entidad por su clave y actualiza solo los elementos de UI afectados.

:::important Compromiso de entidades individuales
...
1. Aplica el predicado a cada ítem en tu colección
2. Crea un flujo filtrado de ítems coincidentes
3. Notifica a los componentes conectados para que actualicen su visualización

El filtro persiste hasta que lo cambies. Los nuevos ítems añadidos a la colección se prueban automáticamente con respecto al filtro actual.


## Trabajar con claves de entidad {#working-with-entity-keys}

El repositorio necesita identificar entidades de manera única para admitir operaciones como `find()` y `commit(entity)`. Hay dos formas de definir cómo se identifican las entidades:
...
Para entidades donde no puedes o no quieres implementar `HasEntityKey` (como entidades JPA), usa `setKeyProvider()`:
