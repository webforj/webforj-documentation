...
En utilisant ces informations, startforJ créera un projet de base à partir de votre archétype choisi configuré pour Spring Boot. 
...
Si vous préférez utiliser la ligne de commande, générez un projet Spring Boot webforJ directement en utilisant les archétypes webforJ officiels :
...
- Dépendance start du Spring Boot pour webforJ
...
Si vous avez une application webforJ existante, vous pouvez ajouter Spring Boot en modifiant la configuration de votre projet. Ce processus consiste à mettre à jour votre configuration Maven, à ajouter des dépendances Spring et à convertir votre classe principale d'application.
...
Ajoutez le starter Spring Boot pour webforJ à vos dépendances. Conservez votre dépendance webforJ existante :
...
:::tip[Outils DevTools pour webforJ pour un rafraîchissement automatique du navigateur]
...
Transformez votre classe principale `App` en une application Spring Boot en ajoutant les annotations Spring nécessaires et une méthode principale :
...
L'annotation `@SpringBootApplication` active l'auto-configuration de Spring et le balayage des composants. L'annotation `@Routify` reste la même, continuant à scanner vos packages de vues pour les routes.
...
# Nom de classe entièrement qualifiée du point d'entrée de l'application
...
# Configuration des outils DevTools pour webforJ
...
Les paramètres `application.properties` suivants pour webforJ sont spécifiques au framework Spring :

| Propriété | Type | Description | Défaut|
|----------|------|-------------|--------|
| **`webforj.servletMapping`** | String | Modèle de chemin d'URL pour le servlet webforJ. | `/*` |
| **`webforj.excludeUrls`** | List | Modèles d'URL qui ne devraient pas être gérés par webforJ lorsqu'ils sont mappés à la racine. Lorsque webforJ est mappé au contexte racine (`/*`), ces modèles d'URL seront exclus de la gestion par webforJ et peuvent être gérés par les contrôleurs Spring MVC à la place. Cela permet aux points de terminaison REST et autres mappages Spring MVC de coexister avec les routes webforJ. | `[]` |
...
| **Configuration de port** | Dans la configuration du plugin | `server.port` dans les propriétés |
