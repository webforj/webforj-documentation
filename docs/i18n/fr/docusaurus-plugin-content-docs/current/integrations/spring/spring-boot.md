...
---
Spring Boot est un choix populaire pour construire des applications Java, offrant l'injection de dépendances, l'auto-configuration et un modèle de serveur embarqué. Lorsque vous utilisez Spring Boot avec webforJ, vous pouvez injecter des services, des dépôts et d'autres beans gérés par Spring directement dans vos composants d'interface utilisateur via l'injection par constructeur.

Lorsque vous utilisez Spring Boot avec webforJ, votre application s'exécute en tant que JAR exécutable avec un serveur Tomcat embarqué au lieu de déployer un fichier WAR sur un serveur d'application externe. Ce modèle d'emballage simplifie le déploiement et s'aligne sur les pratiques de déploiement cloud-native. Le modèle de composants et le routage de webforJ fonctionnent aux côtés du contexte d'application de Spring pour gérer les dépendances et la configuration.
...
À partir de ces informations, startforJ créera un projet de base à partir de l'archétype choisi configuré pour Spring Boot. Vous pouvez choisir de télécharger votre projet sous forme de fichier ZIP ou de le publier directement sur GitHub.
...
Si vous avez une application webforJ existante, vous pouvez ajouter Spring Boot en modifiant votre configuration de projet. Ce processus consiste à mettre à jour votre configuration Maven, à ajouter des dépendances Spring et à convertir votre classe principale d'application.
...
L'annotation `@SpringBootApplication` active l'auto-configuration et la récupération de composants de Spring. L'annotation `@Routify` reste la même, continuant de scanner vos paquets de vues pour les routes.
...
Utilisez le fichier `application.properties` dans `src/main/resources` pour configurer votre application. 
 Voir [Configuration des propriétés](/docs/configuration/properties) pour des informations sur les propriétés de configuration webforJ.
