...
Lorsqu'elles sont installées, elles apparaissent sur l'écran d'accueil ou dans le menu d'applications, de manière similaire aux applications natives. 
Pour ce faire, certaines métadonnées telles que le nom, la description et les icônes doivent être fournies. 
Ces détails aident le système d'exploitation à identifier et afficher l'application.
...
Les navigateurs rejettent les tentatives d'installation sur des origines non sécurisées. Toutefois, cette règle ne s'applique pas lorsque l'application est servie localement depuis `localhost` pendant le développement.
...
Le support pour l'installation d'une application web varie selon le navigateur et la plateforme.

### Bureau {#browser-support-desktop}

- Les **navigateurs basés sur Chromium** (Chrome, Edge, Opera, Brave, et autres) installent n'importe quelle application qui expédie un fichier manifeste sur tous les systèmes d'exploitation de bureau pris en charge.
- **Safari** prend en charge **Fichier → Ajouter au Dock** sur macOS Sonoma (Safari 17) et versions ultérieures. Le flux fonctionne pour toute application web, avec ou sans fichier manifeste.
...
- Sur **Android**, Chrome, Edge, Firefox, Opera et Samsung Internet prennent tous en charge l'installation d'applications web.
...
- Met automatiquement en place les balises méta nécessaires.
- Génère un [Manifeste d'application Web](https://developer.mozilla.org/en-US/docs/Web/Manifest).
- Sert les ressources associées telles que les icônes et les captures d'écran.
...
| `description`     | `String`                                           | Une brève description de l'application, affichée pendant l'installation et dans les paramètres de l'application. | `""`                  |
...
| `defaultIcon`     | [`DefaultIcon`](#appprofiledefaulticon-properties) | Spécifie une icône par défaut pour l'application. Génère automatiquement des chemins d'icône dans plusieurs tailles si configuré. | `icons://icon.png` |
| `screenshots`     | [`Screenshot[]`](#appprofilescreenshot-properties) | Un tableau de captures d'écran pour l'application, utilisées dans les dialogues d'installation.            | `[]`                  |
...
| `type`                                                                           | `String` | Le type média de l'icône (par exemple, `image/png`, `image/jpeg`). Si non fourni, il sera détecté  | `""`              |
...
Elle produit des icônes aux résolutions souvent demandées par les appareils.

| **Propriété** | **Type** | **Description**                                                                 | **Valeur par défaut** |
| ------------ | -------- | ------------------------------------------------------------------------------- | ----------------- |
| `value`      | `String` | Le chemin vers le fichier icône de base. Cela peut être une URL absolue ou un chemin `ws://`. | **Obligatoire**     |
| `sizes`      | `int[]`  | Un tableau de tailles à générer, spécifié comme des entiers (par exemple, `{144, 192, 512}`). | `{144, 192, 512}` |

:::info Exigences concernant les fichiers d'icônes
Cette configuration ne génère pas automatiquement les fichiers d'icônes réels pour l'application. Au lieu de cela, elle utilise l'annotation `@AppProfile.DefaultIcon` pour générer les entrées [`@AppProfile.Icon`](#appprofileicon-properties) correspondantes pour chaque taille spécifiée.

#### Si vous utilisez le [protocole de serveur web](../managing-resources/assets-protocols#the-webserver-protocol) {#if-using-the-webserver-protocol}
- Vous devez fournir un fichier icône de base `icon.png` dans le dossier `static/icons`.
- Vous devez inclure des variations d'icône supplémentaires nommées `icon-144x144.png`, `icon-192x192.png` et `icon-512x512.png`.
- Ces tailles spécifiques couvrent les résolutions que les appareils demandent souvent.

#### Si vous utilisez le [protocole d'icônes](../managing-resources/assets-protocols#the-icons-protocol) {#if-using-the-icons-protocol}

- Vous devez fournir un fichier icône de base `icon.png` dans le dossier `/icons`.
...
| `type`                                                                                        | `String` | Le type média de la capture d'écran (par exemple, `image/png`, `image/jpeg`). Si non fourni, il sera détecté | `""`              |
