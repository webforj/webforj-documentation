...
---
Dans webforJ, toutes les routes sont rendues au sein d'un Frame, qui sert de conteneur de niveau supérieur chargé d'afficher le contenu de la route actuelle. À mesure que les utilisateurs naviguent entre différentes routes, le titre du Frame est mis à jour dynamiquement pour refléter la vue active, aidant ainsi à fournir un contexte clair sur l'emplacement actuel de l'utilisateur dans l'application.

Le titre d'un frame peut être défini soit de manière statique à l'aide d'annotations, soit dynamiquement par le biais de code à l'exécution. Cette approche flexible permet aux développeurs de définir des titres qui s'alignent sur le but de chaque vue, tout en s'adaptant aux scénarios ou paramètres spécifiques si nécessaire.

## Titre du frame avec annotations {#frame-title-with-annotations}

La manière la plus simple de définir le titre d'un frame dans une vue est d'utiliser l'annotation `@FrameTitle`. Cette annotation permet de définir un titre statique pour tout composant de route, qui est ensuite appliqué au frame lorsque le composant est rendu.
...
- L'annotation `@FrameTitle("Dashboard")` définit le titre du frame comme "Dashboard".
- Lorsque l'utilisateur navigue vers `/dashboard`, le titre du frame sera automatiquement mis à jour avec la valeur spécifiée.

Cette méthode est utile pour les routes qui ont un titre statique et qui ne nécessitent pas de mises à jour fréquentes basées sur le contexte de la route.
...
L'interface `HasFrameTitle` contient une seule méthode `getFrameTitle()`, qui est invoquée avant que le titre du frame ne soit mis à jour. Cette méthode offre la flexibilité de générer un titre dynamiquement basé sur le contexte de navigation ou d'autres facteurs dynamiques.
...
- La méthode `getFrameTitle()` génère dynamiquement un titre à l'aide du paramètre `id` de l'URL.
- Si la route est `/profile/123`, le titre sera mis à jour en "Profil - Utilisateur 123".

:::tip Combinaison d'annotations et de titres dynamiques
Vous pouvez combiner des méthodes statiques et dynamiques. Si un composant de route possède à la fois une annotation `@FrameTitle` et implémente l'interface `HasFrameTitle`, le titre fourni dynamiquement par `getFrameTitle()` prendra le pas sur la valeur statique de l'annotation.
:::
