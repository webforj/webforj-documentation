...
:::tip webforJ `Router`
Voir l'[article sur le `Router`](../routing/overview) pour en savoir plus sur la gestion complète des URL et des itinéraires.
:::
...
- `replaceState(Object state, Location location)`: Remplace l'entrée actuelle de l'historique. Cela ne crée pas de nouvelle entrée dans la pile comme la méthode <!-- valeoff -->ci-dessus<!-- valeon -->.
