...
2. **Chargement du service** : Au démarrage de l'application, webforJ charge toutes les classes répertoriées dans `META-INF/services/com.webforj.error.ErrorHandler`.
...
L'interface `ErrorHandler` est conçue pour gérer les erreurs qui se produisent lors de l'exécution d'une application webforJ. Les applications qui souhaitent gérer des exceptions spécifiques doivent mettre en œuvre cette interface.

### Méthodes {#methods}

- **`onError(Throwable throwable, boolean debug)`** : Appelée lorsqu'une erreur se produit. Cette méthode doit contenir la logique pour traiter l'exception.
...
La classe d'implémentation doit être nommée d'après l'exception qu'elle gère, avec le suffixe `ErrorHandler`. Par exemple, pour gérer `NullPointerException`, la classe doit être nommée `NullPointerExceptionErrorHandler`.
...
## Mise en œuvre d'un gestionnaire d'erreurs personnalisé {#implementing-a-custom-error-handler}

Les étapes suivantes détaillent la mise en œuvre d'un gestionnaire d'erreurs personnalisé pour une exception spécifique :

### Étape 1 : Créer la classe du gestionnaire d'erreurs {#step-1-create-the-error-handler-class}

Créer une nouvelle classe qui implémente l'interface `ErrorHandler` et est nommée d'après l'exception qu'elle gère.
...
La méthode `showErrorPage` est une méthode utilitaire qui utilise l'API webforJ pour envoyer le contenu HTML fourni et le titre de la page au navigateur, affichant une page d'erreur. Lorsqu'une exception se produit et que l'application ne parvient pas à récupérer, il devient impossible d'utiliser les composants de webforJ pour construire une page d'erreur personnalisée. Cependant, l'API `Page` reste accessible, permettant au développeur de rediriger ou d'afficher une page d'erreur en dernière tentative.
:::

### Étape 2 : Enregistrer le gestionnaire d'erreurs {#step-2-register-the-error-handler}

Créer un fichier nommé `com.webforj.error.ErrorHandler` dans le répertoire `META-INF/services` de votre application. Ajouter le nom complet de votre classe de gestionnaire d'erreurs dans ce fichier.
...
Maintenant, chaque fois qu'une `NullPointerException` est levée, webforJ sélectionne votre gestionnaire enregistré et exécute sa logique pour gérer l'erreur.
...
- **Affichage de la page d'erreur** : En fonction du mode débogage, la page d'erreur affiche la trace de la pile ou un message d'erreur générique.

### Définition d'un gestionnaire d'erreurs global personnalisé {#defining-a-custom-global-error-handler}

Pour définir un gestionnaire d'erreurs global, vous devez créer un nouveau gestionnaire d'erreurs nommé `WebforjGlobalErrorHandler`. Ensuite, suivez [les étapes pour enregistrer les gestionnaires d'erreurs](#step-2-register-the-error-handler) comme expliqué précédemment. Dans ce cas, webforJ cherchera d'abord des gestionnaires d'erreurs personnalisés pour gérer les exceptions. Si aucun n'est trouvé, webforJ se rabat sur le gestionnaire d'erreurs global personnalisé.
