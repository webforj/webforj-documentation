...
Lors de l'utilisation de la classe `ElementComposite`, la méthode `getElement()` vous donne accès au composant `Element` sous-jacent. De même, la méthode `getNodeName()` vous donne le nom de ce nœud dans le DOM.
...
Les exemples sur cette page démontrent comment implémenter le [composant web de code QR Shoelace](https://shoelace.style/components/qr-code) en utilisant la classe `ElementComposite`.
...
:::info
Les propriétés sont accessibles et manipulées en interne dans le code du composant et ne se reflètent pas dans le DOM. En revanche, les attributs font partie de l'interface externe du composant et peuvent être utilisés pour passer des informations dans un composant depuis l'extérieur, offrant un moyen aux éléments ou scripts externes de configurer le composant.
:::
...
Utilisez la méthode `get()` dans la classe `ElementComposite` pour accéder et lire les propriétés. La méthode `get()` peut accepter une valeur booléenne optionnelle, qui est fausse par défaut, pour indiquer si la méthode doit aller chercher la valeur sur le client. Cela impacte la performance, mais peut être nécessaire si la propriété peut être modifiée uniquement sur le client.

Un `Type` peut également être passé à la méthode, ce qui dicte comment caster le résultat récupéré.

:::tip
Ce `Type` n'est pas explicitement nécessaire et ajoute un niveau supplémentaire de spécification lors de la récupération des données.
:::
...
Dans la démo ci-dessous, des propriétés ont été ajoutées pour le code QR en fonction de la documentation pour le composant web. Des méthodes ont ensuite été mises en œuvre permettant aux utilisateurs d'obtenir et de définir les différentes propriétés qui ont été implémentées.
...
Les événements permettent la communication entre différentes parties de votre application webforJ. La classe `ElementComposite` fournit la gestion des événements avec un support pour le débounce, le throttling, le filtrage et la collecte de données d'événements personnalisées.
...
:::info
Les événements `ElementComposite` sont différents des événements `Element`, en ce sens qu'ils ne permettent aucune classe, mais uniquement des classes `Event` spécifiées.
:::

### Classes d'événements intégrées {#built-in-event-classes}

webforJ fournit des classes d'événements pré-construites avec un accès aux données typées :

- **ElementClickEvent** : Événements de clic de souris avec coordonnées (`getClientX()`, `getClientY()`), informations sur le bouton (`getButton()`) et touches modificatrices (`isCtrlKey()`, `isShiftKey()`, etc.)
...
Les événements transportent des données du client vers votre code Java. Accédez à ces données via `getData()` pour les données d'événements brutes ou utilisez des méthodes typées lorsqu'elles sont disponibles sur les classes d'événements intégrées. Pour plus de détails sur l'utilisation efficace des charges utiles d'événements, consultez le [guide des événements](../building-ui/events).

## Classes d'événements personnalisées {#custom-event-classes}

Pour une gestion d'événements spécialisée, créez des classes d'événements personnalisées avec des charges utiles configurées à l'aide des annotations `@EventName` et `@EventOptions`.
...
**Le débounce** retarde l'exécution jusqu'à ce que l'activité s'arrête :
...
- `BOTH` : Déclenche immédiatement et après une période de silence

## Fusion d'options {#options-merging}

Combinez les configurations d'événements provenant de différentes sources à l'aide de `mergeWith()`. Les options de base fournissent des données communes pour tous les événements, tandis que les options spécifiques ajoutent une configuration spécialisée. Les options ultérieures remplacent les paramètres conflictuels.
...
1. **`findComponentSlot()`** : Cette méthode est utilisée pour rechercher un composant spécifique à travers tous les slots dans un système de composants. Elle retourne le nom du slot où le composant se trouve. Si le composant n'est trouvé dans aucun slot, une chaîne vide est renvoyée.
...
3. **`getFirstComponentInSlot()`** : Cette méthode est conçue pour obtenir le premier composant assigné au slot. En option, passez un type de classe spécifique pour filtrer les résultats de cette méthode.
...
Ces interactions permettent aux développeurs d'exploiter la puissance des composants web en fournissant une API claire et simple pour manipuler des slots, des propriétés et gérer des événements au sein de la classe `ElementComposite`.
