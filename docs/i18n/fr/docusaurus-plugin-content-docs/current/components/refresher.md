...
Le pull-to-refresh est un modèle commun dans les interfaces mobiles et adaptées aux tactiles, et le composant `Refresher` l'apporte aux conteneurs défilables dans webforJ. Lorsque les utilisateurs glissent vers le bas au-delà d'un seuil configurable, il passe par des états visuels : `pull`, `release` et `refreshing`, chacun avec une icône personnalisable et un texte localisé. Il se marie bien avec [`InfiniteScroll`](../components/infinitescroll) pour recharger ou réinitialiser le contenu via des entrées gestuelles.

<!-- INTRO_END -->

## Instantiation et internationalisation {#instantiation-and-internationalization}

Ajoutez un `Refresher` en l'instanciant et en enregistrant un auditeur de rafraîchissement. Lorsque les opérations de rafraîchissement sont terminées, appelez `finish()` pour réinitialiser le composant à son état inactif.

:::info Comment activer le `Refresher`
Pour activer le `Refresher`, **cliquez et faites glisser vers le bas** depuis le haut de la zone défilable. Bien que ce geste soit familier sur mobile, il est moins courant sur bureau—assurez-vous de maintenir et de tirer avec votre souris.
:::
...
## Configuration du comportement de pull {#pull-behavior-configuration}
...
Pour définir la distance maximale de tirage autorisée, utilisez la méthode `setThresholdMax()` :
...
Le composant `Refresher` maintient son propre état interne et communique les changements d'état par le biais d'événements. Lorsqu'un utilisateur tire vers le bas au-delà du seuil défini, le `Refresher` émet un événement de rafraîchissement auquel vous pouvez répondre en enregistrant un auditeur `onRefresh()`.

À l'intérieur de cet auditeur, vous êtes censé effectuer l'opération nécessaire, comme récupérer de nouvelles données ou réinitialiser une liste, puis appeler explicitement :
...
Vous pouvez également désactiver le `Refresher` par programmation à tout moment pour empêcher l'utilisateur de déclencher un comportement de rafraîchissement :

```java
refresher.setEnabled(false);
```

Ceci est utile lorsque les rafraîchissements doivent être temporairement interdits—par exemple, pendant un écran de chargement ou pendant qu'un autre processus critique est en cours d'exécution.

## Style {#styling}

### Thèmes {#themes}

Le composant `Refresher` prend en charge plusieurs thèmes pour distinguer visuellement les différents états ou pour correspondre à l'apparence et à la convivialité de votre application. Les thèmes peuvent être appliqués à l'aide de la méthode `setTheme()`.
