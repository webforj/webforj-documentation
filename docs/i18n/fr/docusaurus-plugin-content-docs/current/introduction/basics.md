...
---
Une fois que webforJ et ses dépendances sont configurés dans votre projet, vous êtes prêt à créer la structure de l'application. Cet article décrira les éléments clés d'une application webforJ de base, en se concentrant spécifiquement sur les classes `Application` et `HomeView`, qui sont les classes fondamentales du projet de démarrage `webforj-archetype-hello-world`.

## Classe principale de l'application : `Application.java` {#main-app-class-applicationjava}

La classe `Application` sert de point d'entrée pour votre application webforJ, configurant les configurations et routes essentielles. Pour commencer, notez la déclaration de la classe et les annotations.

Cette classe étend la classe de base `App` de webforJ, la rendant reconnaissable en tant qu'application webforJ. Plusieurs annotations configurent le thème, le titre et le routage de l'application.
...
Avec `Application.java` configuré, l'application est maintenant configurée avec un titre et des routes pointant vers le package des vues. Ensuite, un aperçu de la classe `HomeView` donne un aperçu de ce qui est affiché lorsque l'application est exécutée.

### Découverte d'un `App` {#discovering-an-app}

Une limite d'une seule <JavadocLink type="foundation" location="com/webforj/App" code='true'>App</JavadocLink> est imposée dans webforJ, ce qui transfère toutes les responsabilités de gestion des erreurs au côté Java et donne aux développeurs un contrôle total sur la gestion des erreurs.

Lors du processus de démarrage de webforJ, toutes les classes qui étendent <JavadocLink type="foundation" location="com/webforj/App" code='true'>com.webforj.App</JavadocLink> sont scannées. Si plusieurs applications sont trouvées, le système recherche l'annotation <JavadocLink type="foundation" location="com/webforj/annotation/AppEntry" code='true'>com.webforj.annotation.AppEntry</JavadocLink>. Si l'une des classes découvertes est annotée avec <JavadocLink type="foundation" location="com/webforj/annotation/AppEntry" code='true'>@AppEntry</JavadocLink>, la première rencontrée est considérée comme le point d'entrée.
...
Pour plus d'informations sur la gestion des erreurs dans webforJ, consultez [cet article](../advanced/error-handling).
:::

## Classe de vue principale : `HomeView.java` {#main-view-class-homeviewjava}

La classe `HomeView` définit un composant de vue simple qui sert de page d'accueil pour l'application. Elle affiche un champ et un bouton pour saluer le nom tapé par l'utilisateur.

### Déclaration de la classe et annotations {#class-declaration-and-annotations}

`HomeView` étend `Composite<FlexLayout>`, ce qui lui permet d'agir en tant que composant réutilisable composé d'un composant [`FlexLayout`](../components/flex-layout). La [`@Route("/")`](../routing/overview) fait de cela la route racine de l'application.
...
public class HelloWorldView extends Composite<FlexLayout> {
  ...
  public HelloWorldView(){
    ...
    btn.setTheme(ButtonTheme.PRIMARY)
        .addClickListener(e -> 
          Toast.show("Bienvenue dans webforJ Starter " + hello.getValue() + "!", Theme.GRAY));
...
- `btn` : Un [`Button`](../components/button) de style principal étiqueté `Dire Bonjour`.
